...
class Guesser(threading.Thread):
	def __init__(self, rows, cols, piece, piece2, name, special, choices):
	...
	def run(self):
		...
		while not self.stopit:
			...
			if a and b:
				...
				seq = (a[0], a[1], b[0], b[1], rot)
				if not seq in self._choices:
					a = None

			if not a or not b or (last[0] == a and last[1] == b and last[2] == rot):
				seq = random.choice(self._choices)

			else:
				last = (a, b, rot)

			data = "%d %d %d %d %d" % (seq[1] + 1, seq[0] + 1, seq[3] + 1, seq[2] + 1, seq[4])

			try:
				self._d24.command("GUESS " + data)
				try:
					self._choices.remove(seq)
...
class Guesser3(threading.Thread):
	...
	def run(self):
		self._d24.debug('Running!')
		while not self.stopit:
			self._d24.debug('ZOSTALO: %d' % len(self._choices))
			seq = random.choice(self._choices)
			data = "%d %d %d %d %d" % (seq[1] + 1, seq[0] + 1, seq[3] + 1, seq[2] + 1, seq[4])
...
class Builder(threading.Thread):
	def __init__(self, name, piece, piece2, choices):
	...
	def run(self):
		...
		while True:
			...
			for g in xrange(good):
				...
				drot = int(drot)
				count = int(count[1:-1])
				for i in (0, 90, 270, 280):
					try:
						self._choices.remove((row, col, drow, dcol, i))
...
while True:
	try:
		...
		cols = int(cols)
		rows = int(rows)
		choices = [(fr, fc, tr, tc, rot * 90) for fr in xrange(rows) for fc in xrange(cols) for tr in xrange(rows) for tc in xrange(cols) for rot in xrange(4)]
		...
		special = [False, False]
		build = Builder(name, piece, piece2, choices)
		guess = Guesser(rows, cols, piece, piece2, name, special, choices)
		#guess2 = Guesser2(rows, cols, piece, piece2, name, special)
		guess3 = Guesser3(rows, cols, piece, piece2, name, special, choices)
