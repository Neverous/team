...
class Game(object):
	...
	def getBestTime(self, ed):
		edges = self.roads
		best = 0
		for e in xrange(len(ed)):
			e = ed[e]
			if not self.isMine[edges[e].getId()] and edges[e].getCost() > 100:
				continue

			if edges[e].getLength() / edges[e].getSpeed() < edges[best].getLength() / edges[best].getSpeed():
				best = e

		if not self.isMine[edges[best].getId()] and edges[best].getCost() > 100:
			return -1

		return edges[best].getLength() / edges[best].getSpeed()
	...
	def countVertices(self):
		...
		while queue:
			cost, act = queue.pop()
			if self.distance[act] < cost:
				continue

			for neighbour in self.parsed[act].keys():
				tt = self.getBestTime(self.connections[act][neighbour])
				if tt == -1:
					continue

				if self.distance[neighbour] <= self.distance[act] + tt:
					continue

				self.distance[neighbour] = self.distance[act] + tt
				queue.add(pair((self.distance[neighbour], neighbour)))
	...
	def run(self, autko):
		...
		while True:
			...
			if not route and self.actZuk.getUsed() and not random.randint(0, 2):
					route = self.getShortestPath(self.actZuk.getPlace(), self.actZuk.people.values()[0].getDestination())
					if route == -1:
			...
			if not route:
				...
				if self.actZuk.getFree():
					...
					self.countVertices()
					best = self.actZuk.people.values()[0]
					route = self.getShortestPath(self.actZuk.getPlace(), best.getDestination())
					print 'R1', route
					if route == -1:
						something = sorted(self.getEdgesFrom(self.actZuk.getPlace(), self.actZuk.visited), self.sortEdges)
						#print 'POSORTOWANE2:'
			...
			try:
				...
				if not self.isMine[sth[best]] and self.roads[sth[best]].getCost() > 100:
					best
